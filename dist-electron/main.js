...
ipcMain.handle("create-commits", async (event, { repoPath, commits, autoPush, branch }) => {
  try {
    const git = esm_default(repoPath);
    ...
    for (const commit of commits) {
      ...
      const commitResult = await git.commit(commit.message, {
        "--date": commit.date
      });
      createdCommits.push({
    ...
    return { success: true, commits: createdCommits };
  } catch (error) {
    return { success: false, error: error.message };
  }
});
